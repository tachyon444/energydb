...
/**
 * India BEE HVAC crawler (PDF + Approval Date + 4/5 Star filter version)
 *
 * Flow:
 ...
 *   INDIA_STAR_RATINGS            default 4,5; comma-separated star filters; use all/none to disable
 *
 * Fix note: DEFAULT_CATEGORIES/HVAC_CATEGORY_MAP are initialized before CONFIG so INDIA_CATEGORIES works.
 * Default operation filters each category to 4-Star and 5-Star PDFs to reduce BEE load and keep high-efficiency competitor focus.
 */
...
const CONFIG = {
  ...
  starRatings: parseStarRatingsEnv(process.env.INDIA_STAR_RATINGS, ['4', '5']),
};
...
function parseStarRatingsEnv(value, fallback) {
  if (value == null || String(value).trim() === '') return fallback.slice();
  const raw = String(value).trim().toLowerCase();
  if (['all', 'none', 'false', '0', 'off'].includes(raw)) return [];
    ...
    const result = await page.evaluate(({ passNo, filterOverrides }) => {
      ...
      selects.forEach((select, idx) => {
        ...
        if (looksLikeStarRatingSelect) {
          ...
          // BEE uses a Select All sentinel option. Select the sentinel itself, not thousands of individual options.
          selectAllOpt.selected = true;
          selectedLabels = [norm(selectAllOpt.textContent || selectAllOpt.label || selectAllOpt.value || 'Select All')];
        } else {
...
function didApplyStarRatingFilter(selectedInfos, starRating) {
  const target = String(starRating || '').trim();
  if (!target) return false;
  const infos = Array.isArray(selectedInfos) ? selectedInfos : [];
  return infos.some((info) => {
    if (!info || info.mode !== 'star-rating-single') return false;
    const labels = Array.isArray(info.selectedLabels) ? info.selectedLabels : [];
    return labels.some((label) => new RegExp('^' + escapeRegExp(target) + '(\\s*star|\\s*$)', 'i').test(String(label || '').trim()));
  });
}
...
async function downloadCategoryPdf(page, category, filterOverrides) {
  ...
  const fileName = `${slugify(category.label)}${starSuffix}_${new Date().toISOString().replace(/[-:.TZ]/g, '').slice(0, 14)}.pdf`;
...
async function scrapeCategoryViaPdf(page, category, reportWindow, runId) {
  const ratings = CONFIG.starRatings || [];
  if (!ratings.length) {
  ...
  const combined = {
    status: 'ok',
    message: `star-rating-filter=${ratings.join(',')}`,
    pdfPath: '',
    scrapedRows: 0,
    keptRows7d: 0,
    records: [],
  ...
  for (const rating of ratings) {
    console.log(`[INDIA] star rating filter: ${category.label} / ${rating}`);
    try {
      ...
      console.error(`[INDIA] star rating filter failed: ${category.label} / ${rating} / ${msg}`);
...
async function scrapeCategoryPdfOnce(page, category, reportWindow, runId, filterOverrides) {
  ...
  if (filterOverrides.starRating && !didApplyStarRatingFilter(selectedInfos, filterOverrides.starRating)) {
    throw new Error(`Star Rating filter ${filterOverrides.starRating} was not applied. Refusing to tag unfiltered rows as ${filterOverrides.starRating}-Star.`);
  }
  await clickIndiaFilterSubmit(page, selectedInfos);
  await waitForResultsReady(page);
  const starSuffix = filterOverrides.starRating ? `_star_${filterOverrides.starRating}` : '';
  await saveDebugArtifacts(page, `${slugify(category.label)}${starSuffix}_results`);

  const pdfPath = await downloadCategoryPdf(page, category, filterOverrides);
...
function parsePdfRowBlock(block, category) {
  ...
  for (const line of lines) {
    if (extractDateTokens(line).length) {
      const modelPrefix = extractModelPrefixBeforeDates(line);
      if (modelPrefix) preDateLines.push(modelPrefix);
      break;
    }
  ...
  const modelName = normalizeModelName(modelLines.join(' '));
...
function looksLikeTypeStartLine(line) {
  ...
  if (isSpeedQualifierLine(lower) || isPartialSpeedQualifierLine(lower)) return true;
  return [
    'split',
    'window',
    'cassette',
...
function extractModelPrefixBeforeDates(line) {
  const text = normalizeText(line);
  const dates = extractDateTokens(text);
  if (!dates.length) return '';

  const firstDateIndex = text.indexOf(dates[0]);
  if (firstDateIndex < 0) return '';

  let prefix = normalizeText(text.slice(0, firstDateIndex));

  // Variable Speed PDFs often place model + numeric metrics + dates on one line:
  // "MSY-JR36VF 4 2.8 1936.28 12/2/2025 12/31/2027".
  // Keep the model prefix and strip trailing numeric metric tokens.
  for (let i = 0; i < 8; i++) {
    const next = normalizeText(prefix.replace(/(?:^|\s)\d+(?:\.\d+)?\s*$/, ''));
    if (next === prefix) break;
    prefix = next;
  }

  if (!/[A-Za-z]/.test(prefix)) return '';
  if (isSpeedQualifierLine(prefix) || isPartialSpeedQualifierLine(prefix)) return '';
  return cleanModelLine(prefix);
}
...
function isSpeedQualifierLine(line) {
  const lower = normalizeText(line).toLowerCase();
  return /^\(?\s*(fixed|variable)\s+speed\s*\)?$/.test(lower);
}

function isPartialSpeedQualifierLine(line) {
  const lower = normalizeText(line).toLowerCase();
  return ['(fixed', 'fixed', 'speed)', 'speed', '(variable', 'variable'].includes(lower);
}
